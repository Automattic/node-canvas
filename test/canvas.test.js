...
describe("Canvas", function () {
    ...
    it("registerFont", function () {
        ...
        registerFont("./examples/pfennigFont/Pfennig.ttf", {
            family: "Pfennig",
        });
        registerFont("./examples/pfennigFont/PfennigBold.ttf", {
            family: "Pfennig",
            weight: "bold",
        });

        // Test to multi byte file path support
        registerFont("./examples/pfennigFont/pfennigMultiByte🚀.ttf", {
            family: "Pfennig",
        });
    ...
    it("color serialization", function () {
        ...
        ["fillStyle", "strokeStyle", "shadowColor"].forEach(function (prop) {
            ...
            assert.equal(
                "#ffffff",
                ctx[prop],
                prop + " #FFFFFF -> #ffffff, got " + ctx[prop]
            );
            ...
            assert.equal(
                "#80c880",
                ctx[prop],
                prop + " rgba(128, 200, 128, 1) -> #80c880, got " + ctx[prop]
            );
            ...
            assert.equal(
                ...
                ctx[prop],
                prop +
                    " rgba(128,80,0,0.5) -> rgba(128, 80, 0, 0.5), got " +
                    ctx[prop]
            );
            ...
            assert.equal(
                ...
                ctx[prop],
                prop +
                    " rgba(128,80,0,0.75) -> rgba(128, 80, 0, 0.75), got " +
                    ctx[prop]
            );
    ...
    it("Canvas#width= (resurfacing) doesn't crash when fillStyle is a pattern (#1357)", function (done) {
        ...
        loadImage(path.join(__dirname, "/fixtures/checkers.png")).then(
            (img) => {
    ...
    describe("#toDataURL()", function () {
        ...
        it("toDataURL(0.5) works and defaults to PNG", function () {
            assert.ok(
                canvas.toDataURL(0.5).startsWith("data:image/png;base64,")
            );
        });

        it("toDataURL(undefined) works and defaults to PNG", function () {
            assert.ok(
                canvas.toDataURL(undefined).startsWith("data:image/png;base64,")
            );
        });
        ...
        it('toDataURL("image/png", 0.5, function (err, str) {...}) works', function (done) {
            createCanvas(200, 200).toDataURL(
                "image/png",
                0.5,
                function (err, str) {
                    assert.ifError(err);
    ...
    describe("Context2d#measureText()", function () {
        it("Context2d#measureText().width", function () {
            ...
            assert.ok(
                ctx.measureText("foo").width !== ctx.measureText("foobar").width
            );
            assert.ok(
                ctx.measureText("foo").width !== ctx.measureText("  foo").width
            );
        });
    ...
    it("Context2d#createPattern(Image)", async function () {
        const img = await loadImage(
            path.join(__dirname, "/fixtures/checkers.png")
        );
        const canvas = createCanvas(20, 20);
        const ctx = canvas.getContext("2d");
    ...
    describe("Context2d#putImageData()", function () {
        it("throws for invalid arguments", function () {
            ...
            assert.throws(function () {
                ctx.putImageData(undefined, 0, 0);
            }, TypeError);
        });
        ...
        it("throws if canvas is a PDF canvas (#1853)", function () {
            const canvas = createCanvas(3, 6, "pdf");
            const ctx = canvas.getContext("2d");
            const srcImageData = createImageData(
    ...
    describe("Context2d#save()/restore()", function () {
        ...
        const state = [
            // non-default values to test with
            ["strokeStyle", "#ff0000"],
            ["fillStyle", "#ff0000"],
            ["globalAlpha", 0.5],
            ["lineWidth", 0.5],
