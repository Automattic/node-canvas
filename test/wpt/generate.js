...
function convert(test) {
    ...
    code = code.replace(
        /@assert throws (\S+_ERR) (.*);/g,
        "assert.throws(function() { $2; }, /$1/);"
    );

    code = code.replace(
        /@assert throws (\S+Error) (.*);/g,
        "assert.throws(function() { $2; }, $1);"
    );

    code = code.replace(/@assert (.*) === (.*);/g, (match, g1, g2) => {
        return `assert.strictEqual(${g1}, ${g2}, "${escapeJS(g1)}", "${escapeJS(
            g2
        )}")`;
    });

    code = code.replace(/@assert (.*) !== (.*);/g, (match, g1, g2) => {
        return `assert.notStrictEqual(${g1}, ${g2}, "${escapeJS(
            g1
        )}", "${escapeJS(g2)}");`;
    });
...
for (const filename of yamlFiles) {
    ...
    const out = fs.createWriteStream(
        `${__dirname}/generated/${filename.replace(".yaml", ".js")}`
    );
