...
function _assertApprox(actual, expected, epsilon = 0, msg = "") {
    assert(
        ...
        "actual should be a number but got a ${typeof type_actual}"
    );
...
function _assertPixel(canvas, x, y, r, g, b, a, pos, color) {
    ...
    assert.strictEqual(
        c[1],
        g,
        "Green channel of the pixel at (" + x + ", " + y + ")"
    );
    assert.strictEqual(
        c[2],
        b,
        "Blue channel of the pixel at (" + x + ", " + y + ")"
    );
    assert.strictEqual(
        c[3],
        a,
        ...
        document.fonts.ready.then(() => {
            step_timeout(
                t.step_func_done(function () {
                    ...
                    assert.strictEqual(
                        Math.abs(ctx.measureText("Hello").advances[0]),
                        0,
                        "Math.abs(ctx.measureText('Hello').advances[\"" +
                            0 +
                            '"])',
                        "0"
                    );
                    ...
                    assert(
                        ctx.measureText("Hello").advances[1] >= 36,
                        "ctx.measureText('Hello').advances[\"" + 1 + '"] >= 36'
                    );
                    assert(
                        ctx.measureText("Hello").advances[2] >= 58,
                        "ctx.measureText('Hello').advances[\"" + 2 + '"] >= 58'
                    );
                    assert(
                        ctx.measureText("Hello").advances[3] >= 70,
                        "ctx.measureText('Hello').advances[\"" + 3 + '"] >= 70'
                    );
                    assert(
                        ctx.measureText("Hello").advances[4] >= 80,
                        "ctx.measureText('Hello').advances[\"" + 4 + '"] >= 80'
                    );

                    var tm = ctx.measureText("Hello");
                    assert.strictEqual(
                        ctx.measureText("Hello").advances[0],
                        tm.advances[0],
                        "ctx.measureText('Hello').advances[\"" + 0 + '"]',
                        'tm.advances["' + 0 + '"]'
                    );
                    assert.strictEqual(
                        ctx.measureText("Hello").advances[1],
                        tm.advances[1],
                        "ctx.measureText('Hello').advances[\"" + 1 + '"]',
                        'tm.advances["' + 1 + '"]'
                    );
                    assert.strictEqual(
                        ctx.measureText("Hello").advances[2],
                        tm.advances[2],
                        "ctx.measureText('Hello').advances[\"" + 2 + '"]',
                        'tm.advances["' + 2 + '"]'
                    );
                    assert.strictEqual(
                        ctx.measureText("Hello").advances[3],
                        tm.advances[3],
                        "ctx.measureText('Hello').advances[\"" + 3 + '"]',
                        'tm.advances["' + 3 + '"]'
                    );
                    assert.strictEqual(
                        ctx.measureText("Hello").advances[4],
                        tm.advances[4],
                        "ctx.measureText('Hello').advances[\"" + 4 + '"]',
                        'tm.advances["' + 4 + '"]'
                    );
        ...
        document.fonts.ready.then(() => {
            step_timeout(
                t.step_func_done(function () {
                    ...
                    assert(
                        Math.abs(ctx.measureText("A").actualBoundingBoxLeft) <=
                            1,
                        "Math.abs(ctx.measureText('A').actualBoundingBoxLeft) <= 1"
                    );
                    assert(
                        ctx.measureText("A").actualBoundingBoxRight >= 50,
                        "ctx.measureText('A').actualBoundingBoxRight >= 50"
                    );
                    assert(
                        ctx.measureText("A").actualBoundingBoxAscent >= 35,
                        "ctx.measureText('A').actualBoundingBoxAscent >= 35"
                    );
                    assert(
                        Math.abs(
                            ctx.measureText("A").actualBoundingBoxDescent
                        ) <= 1,
                        "Math.abs(ctx.measureText('A').actualBoundingBoxDescent) <= 1"
                    );

                    assert(
                        ctx.measureText("D").actualBoundingBoxLeft >= 48,
                        "ctx.measureText('D').actualBoundingBoxLeft >= 48"
                    );
                    assert(
                        ctx.measureText("D").actualBoundingBoxLeft <= 52,
                        "ctx.measureText('D').actualBoundingBoxLeft <= 52"
                    );
                    assert(
                        ctx.measureText("D").actualBoundingBoxRight >= 75,
                        "ctx.measureText('D').actualBoundingBoxRight >= 75"
                    );
                    assert(
                        ctx.measureText("D").actualBoundingBoxRight <= 80,
                        "ctx.measureText('D').actualBoundingBoxRight <= 80"
                    );
                    assert(
                        ctx.measureText("D").actualBoundingBoxAscent >= 35,
                        "ctx.measureText('D').actualBoundingBoxAscent >= 35"
                    );
                    assert(
                        ctx.measureText("D").actualBoundingBoxAscent <= 40,
                        "ctx.measureText('D').actualBoundingBoxAscent <= 40"
                    );
                    assert(
                        ctx.measureText("D").actualBoundingBoxDescent >= 12,
                        "ctx.measureText('D').actualBoundingBoxDescent >= 12"
                    );
                    assert(
                        ctx.measureText("D").actualBoundingBoxDescent <= 15,
                        "ctx.measureText('D').actualBoundingBoxDescent <= 15"
                    );

                    assert(
                        Math.abs(
                            ctx.measureText("ABCD").actualBoundingBoxLeft
                        ) <= 1,
                        "Math.abs(ctx.measureText('ABCD').actualBoundingBoxLeft) <= 1"
                    );
                    assert(
                        ctx.measureText("ABCD").actualBoundingBoxRight >= 200,
                        "ctx.measureText('ABCD').actualBoundingBoxRight >= 200"
                    );
                    assert(
                        ctx.measureText("ABCD").actualBoundingBoxAscent >= 85,
                        "ctx.measureText('ABCD').actualBoundingBoxAscent >= 85"
                    );
                    assert(
                        ctx.measureText("ABCD").actualBoundingBoxDescent >= 37,
                        "ctx.measureText('ABCD').actualBoundingBoxDescent >= 37"
                    );
                }),
                500
        ...
        document.fonts.ready.then(() => {
            step_timeout(
                t.step_func_done(function () {
                    ...
                    ctx.align = "left";
                    assert.strictEqual(
                        ctx.measureText("A").fontBoundingBoxAscent,
                        85,
                        "ctx.measureText('A').fontBoundingBoxAscent",
                        "85"
                    );
                    ...
                    assert.strictEqual(
                        ctx.measureText("ABCD").fontBoundingBoxAscent,
                        85,
                        "ctx.measureText('ABCD').fontBoundingBoxAscent",
                        "85"
                    );
                    assert.strictEqual(
                        ctx.measureText("ABCD").fontBoundingBoxDescent,
                        39,
                        "ctx.measureText('ABCD').fontBoundingBoxDescent",
                        "39"
                    );
        ...
        document.fonts.ready.then(() => {
            step_timeout(
                t.step_func_done(function () {
                    ...
                    ctx.align = "left";
                    assert.strictEqual(
                        ctx.measureText("A").emHeightAscent,
                        37.5,
                        "ctx.measureText('A').emHeightAscent",
                        "37.5"
                    );
                    assert.strictEqual(
                        ctx.measureText("A").emHeightDescent,
                        12.5,
                        "ctx.measureText('A').emHeightDescent",
                        "12.5"
                    );
                    assert.strictEqual(
                        ctx.measureText("A").emHeightDescent +
                            ctx.measureText("A").emHeightAscent,
                        50,
                        "ctx.measureText('A').emHeightDescent + ctx.measureText('A').emHeightAscent",
                        "50"
                    );
                    ...
                    assert.strictEqual(
                        ctx.measureText("ABCD").emHeightDescent,
                        12.5,
                        "ctx.measureText('ABCD').emHeightDescent",
                        "12.5"
                    );
                    assert.strictEqual(
                        ctx.measureText("ABCD").emHeightDescent +
                            ctx.measureText("ABCD").emHeightAscent,
                        50,
                        "ctx.measureText('ABCD').emHeightDescent + ctx.measureText('ABCD').emHeightAscent",
                        "50"
                    );
                }),
                500
        ...
        document.fonts.ready.then(() => {
            step_timeout(
                t.step_func_done(function () {
                    ...
                    assert.strictEqual(
                        Math.abs(
                            ctx.measureText("A").getBaselines().alphabetic
                        ),
                        0,
                        "Math.abs(ctx.measureText('A').getBaselines().alphabetic)",
                        "0"
                    );
                    assert.strictEqual(
                        ctx.measureText("A").getBaselines().ideographic,
                        -39,
                        "ctx.measureText('A').getBaselines().ideographic",
                        "-39"
                    );
                    assert.strictEqual(
                        ctx.measureText("A").getBaselines().hanging,
                        68,
                        "ctx.measureText('A').getBaselines().hanging",
                        "68"
                    );
                    ...
                    assert.strictEqual(
                        Math.abs(
                            ctx.measureText("ABCD").getBaselines().alphabetic
                        ),
                        0,
                        "Math.abs(ctx.measureText('ABCD').getBaselines().alphabetic)",
                        "0"
                    );
                    assert.strictEqual(
                        ctx.measureText("ABCD").getBaselines().ideographic,
                        -39,
                        "ctx.measureText('ABCD').getBaselines().ideographic",
                        "-39"
                    );
                    assert.strictEqual(
                        ctx.measureText("ABCD").getBaselines().hanging,
                        68,
                        "ctx.measureText('ABCD').getBaselines().hanging",
                        "68"
                    );
    ...
    it("2d.text.drawing.style.letterSpacing.measure", function () {
        ...
        function test_letter_spacing(value, difference_spacing, epsilon) {
            ...
            assert_approx_equals(
                ...
                width_normal + difference_spacing,
                epsilon,
    ...
    it("2d.text.drawing.style.wordSpacing.measure", function () {
        ...
        function test_word_spacing(value, difference_spacing, epsilon) {
            ...
            width_with_word_spacing =
                ctx.measureText("Hello World, again").width;
            assert_approx_equals(
                width_with_word_spacing,
                width_normal + difference_spacing,
                epsilon,
                "word spacing doesn't work."
            );
        }
