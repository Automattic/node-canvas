...
tests["fillRect()"] = function (ctx) {
    ...
    function getPointColour(x, y) {
        ...
        return (
            "rgb(" +
            Math.round(brightness * 127.5 * (1 - y3d)) +
            "," +
...
tests["createPattern() then setTransform with no-repeat"] = function (
    ctx,
    done
) {
    var img = new Image();
    img.onload = function () {
        ...
        pattern.setTransform(
            new DOMMatrix().translate(800, 800).rotate(45).scale(4)
        );
        ctx.fillRect(800, 800, 800, 800);
        done();
    };
...
tests["fillText() maxWidth argument + textAlign center (#1253)"] = function (
    ctx
) {
    ctx.font = "Helvetica, sans";
    ctx.textAlign = "center";
...
tests["textAlign right"] = function (ctx) {
    ...
    ctx.fillText("الحق ltr", 100, 100);
    ctx.direction = "rtl";
    ctx.fillText("right rtl", 100, 130);
    ctx.fillText("rtl الحق", 100, 160);
};
...
tests["textAlign left"] = function (ctx) {
    ...
    ctx.fillText("تركت ltr", 100, 100);
    ctx.direction = "rtl";
    ctx.fillText("left rtl", 100, 130);
    ctx.fillText("rtl تركت", 100, 160);
};
...
tests["textAlign start"] = function (ctx) {
    ...
    ctx.fillText("بداية ltr", 100, 100);
    ctx.direction = "rtl";
    ctx.fillText("start rtl", 100, 130);
    ctx.fillText("rtl بداية", 100, 160);
};
...
tests["textAlign end"] = function (ctx) {
    ...
    ctx.fillText("نهاية ltr", 100, 100);
    ctx.direction = "rtl";
    ctx.fillText("start rtl", 100, 130);
    ctx.fillText("rtl نهاية", 100, 160);
};
...
gco.forEach((op) => {
    tests["9 args, transform, globalCompositeOperator " + op] = function (
        ctx,
        done
    ) {
        const img1 = new Image();
...
tests["createPattern() with globalAlpha and smoothing off scaling down"] =
    function (ctx, done) {
        const img = new Image();
        img.onload = function () {
            ctx.imageSmoothingEnabled = false;
...
tests["createPattern() with globalAlpha and smoothing off scaling up"] =
    function (ctx, done) {
        const img = new Image();
        img.onload = function () {
            ctx.imageSmoothingEnabled = false;
...
tests[
    "smoothing and gradients (gradients are not influenced by patternQuality)"
] = function (ctx) {
...
tests["drawImage(img,x,y,w,h) scale down in a scaled up context"] = function (
    ctx,
    done
) {
    const img = new Image();
