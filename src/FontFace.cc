  ...
  env(info.Env())
{
  ...
  if (info[1].IsString()) {
    ...
    size_t len;
#ifdef _WIN32
    napi_get_value_string_utf16(env, url, nullptr, 0, &len);
#else
    napi_get_value_string_utf8(env, url, nullptr, 0, &len);
#endif
    len++;
    descriptor.url = std::make_unique<file_char[]>(len);
#ifdef _WIN32
    napi_get_value_string_utf16(env, url, (char16_t*)descriptor.url.get(), len, nullptr);
#else
...
FontFace::GetStatus(const Napi::CallbackInfo& info) {
  switch (descriptor.status) {
    case FontStatus::Unloaded: return Napi::String::New(env, "unloaded");
    case FontStatus::Loaded: return Napi::String::New(env, "loaded");
    case FontStatus::Error: return Napi::String::New(env, "error");
  }
}
