 ...
 */
double getBaselineAdjustment(text_baseline_t baseline, FontMetrics metrics) {
  switch (baseline) {
  case TEXT_BASELINE_ALPHABETIC:
    return 0;
  case TEXT_BASELINE_TOP:
    return metrics.central + metrics.central / 2;
...
layoutText(
    ...
    while (shapingWorkList.size() && matchIndex < matches.size()) {
      ...
      if (!foundFont) {
        ...
        bool hasIdce = hb_ot_layout_get_baseline(
          ...
          itemizer.bidi_state.level & 1 ? HB_DIRECTION_RTL : HB_DIRECTION_LTR,
        ...
        do {
          ...
          if (isBreak) {
            ...
            if (glyphsNeedReshape == segment.needsReshape) {
              ...
              flushSegment(segment, shapingWorkList, layout.runs, level, face, buffer, isLastMatch);
  ...
  layout.anchorY = getBaselineAdjustment(state->textBaseline, metrics);
  ...
  for (GlyphRun& run : layout.runs) {
    ...
    for (Glyph& glyph : run.glyphs) {
      lineWidth += glyph.x_advance * toPx;
    }
  }

  switch (resolveTextAlignment(state)) {
    case TEXT_ALIGNMENT_CENTER:
      layout.anchorX = lineWidth / 2;
      break;
    case TEXT_ALIGNMENT_RIGHT:
      layout.anchorX = lineWidth;
      break;
