...
cairo_status_t canvas_write_to_pdf_stream(cairo_surface_t *surface, cairo_write_func_t write_func, PdfStreamInfo* streaminfo) {
  size_t whole_chunks = streaminfo->len / CANVAS_PAGE_SIZE;
  size_t remainder = streaminfo->len - whole_chunks * CANVAS_PAGE_SIZE;

  for (size_t i = 0; i < whole_chunks; ++i) {
    write_func(streaminfo, &streaminfo->data[i * CANVAS_PAGE_SIZE], CANVAS_PAGE_SIZE);
  }

  if (remainder) {
    write_func(streaminfo, &streaminfo->data[whole_chunks * CANVAS_PAGE_SIZE], remainder);
  }
...
static uint32_t getSafeBufSize(Canvas* canvas) {
  ...
  return (std::min)((uint32_t)canvas->getWidth() * canvas->getHeight() * 4, (uint32_t)CANVAS_PAGE_SIZE);
}
