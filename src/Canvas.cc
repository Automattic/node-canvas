...
Canvas::Initialize(Handle<Object> target) {
  ...
  target->Set(Nan::New("Canvas").ToLocalChecked(), ctor->GetFunction());
}
...
Canvas::EIO_AfterToBuffer(eio_req *req) {
  ...
  if (closure->status) {
    ...
    Local<Object> buf = Nan::NewBuffer((char*)closure->data, closure->len).ToLocalChecked();
    memcpy(Buffer::Data(buf), closure->data, closure->len);
    Local<Value> argv[2] = { Nan::New(Nan::Null()), buf };
...
NAN_METHOD(Canvas::ToBuffer) {
  ...
  if (info.Length() > 1 && !(info[1]->StrictEquals(Nan::Undefined()) && info[2]->StrictEquals(Nan::Undefined()))) {
    if (!info[1]->StrictEquals(Nan::Undefined())) {
        bool good = true;
        if (info[1]->IsNumber()) {
    ...
    if (!info[2]->StrictEquals(Nan::Undefined())) {
      if (info[2]->IsUint32()) {
...
streamPNG(void *c, const uint8_t *data, unsigned len) {
  Nan::HandleScope scope;
  closure_t *closure = (closure_t *) c;
  Local<Object> buf = Nan::NewBuffer((char *)data, len).ToLocalChecked();
  Local<Value> argv[3] = {
      Nan::New(Nan::Null())
    , buf
    , Nan::New<Number>(len) };
...
NAN_METHOD(Canvas::StreamPNGSync) {
  ...
  if (try_catch.HasCaught()) {
    ...
    Nan::MakeCallback(Nan::GetCurrentContext()->Global(), closure.fn, 1, argv);
  } else {
    Local<Value> argv[3] = {
        Nan::New(Nan::Null())
      , Nan::New(Nan::Null())
...
NAN_METHOD(Canvas::StreamJPEGSync) {
  ...
  Canvas *canvas = Nan::ObjectWrap::Unwrap<Canvas>(info.This());
  closure_t closure;
  closure.fn = Handle<Function>::Cast(info[3]);
